...
void setCalibEnv::getParamFunc(ros::NodeHandle &priv_nh) {
    ...
    priv_nh.param<int>("view_num", view_num_threshold_, 10);

    priv_nh.param<bool>("RT_debug", RT_debug_, false);
    priv_nh.param<bool>("Intrinsic_debug", intrinsic_debug_, true);
    priv_nh.param<std::string>("result_file_intrinsic", result_file_intrinsic_, std::string("/home/catkin_ws/src/mono_cam_calib/result/intrinsic_param.txt"));
